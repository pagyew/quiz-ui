...
`createQuizUI(element, config, actions)` does not create an engine or access storage. Supply action callbacks: `submit`, `hint`, `reset`, `sprint`, `settings`, `theme`, `sound`, `share`, and `giveUp`. The settings callback returns true when applied or throws a validation error to display in the dialog.
